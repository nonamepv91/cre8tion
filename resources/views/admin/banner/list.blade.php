      ...
      <table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable myTable" id="checkAll">
        ...
        <tbody class="list_item">
          ...
          <tr class='row_9'>
            ...
            <td class="textC"><img src="uploads/banner/{{$item->image_link}}" width="300px"></td>
            <td class="textC">{{$item->name}}</td>
            <td class="textR">{{$item->link}}</td>
            <td class="textR">{{$item->location}}</td>
            @if($item->status==1)
            ...
            <td class="option textC">
                <a href="{{route('banner.getedit',$item->id)}}" title="Chỉnh sửa" class="tipS"> 
                  <img src="admin_assets/images/icons/color/edit.png" /> 
                </a> 
                <a href="{{route('banner.delete',$item->id)}}" title="Xóa" class="tipS verify_action" > 
